...
export default function HomeFieldScene() {
  const { lang, t } = useLanguage();
  const stats = [
    { value: '11', label: t('个孩子', 'young people') },
    { value: '9', label: t('个家庭', 'families') },
    { value: t('5 天 4 夜', '5 days, 4 nights'), label: t('共同生活', 'living together') },
  ];
  ...
  return (
    <section id="home-field-scene" className="home-editorial-section home-field-scene">
      <div className="home-editorial-shell home-field-scene__topline">
        <h2
          className="home-field-scene__headline"
          aria-label={t('最近，我们一起生活了5天。', 'Recently, we lived together for 5 days.')}
        >
          {lang === 'zh' ? (
            <>
              最近，我们一起生活了<span className="home-field-scene__headline-number">5</span>天。
            </>
          ) : (
            <>
              Recently, we lived together for <span className="home-field-scene__headline-number">5</span> days.
            </>
          )}
        </h2>
      </div>
      ...
      <div className="home-editorial-shell home-field-scene__story">
        <dl className="home-field-scene__stats">
          {stats.map((stat) => (
